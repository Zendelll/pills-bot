...
class TgClient:
    ...
    async def send_message(self, chat_id: int, text: str, reply_markup: ReplyKeyboardMarkup = None) -> SendMessageResponse:
        ...
        if reply_markup:
            payload["reply_markup"] = reply_markup.Schema().dump(reply_markup)
        async with aiohttp.ClientSession() as session:
